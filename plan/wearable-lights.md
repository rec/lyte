...
Issue #3: build a front-and-side-visible wearable from 250 Twinkly Generation
Dots. The unmodified factory string splits into two 125-dot paths at its tee;
...
- The factory tee and both 125-dot strands remain unmodified. The plan concerns
  only how the supplied string is attached to the garment and how its existing
  output indices map to the wearer.

## Dot Allocation

Use two mirror-image paths of exactly 125 dots. This makes routing from the
...
| --- | ---: | ---: | ---: |
| Arm, wrist to shoulder | 35 | 35 | 70 |
| Leg, ankle to hip | 40 | 40 | 80 |
| Side body, hip to underarm | 20 | 20 | 40 |
| Front chest spiral | 30 | 30 | 60 |
| Total | 125 | 125 | 250 |
...
The two 30-dot chest tails continue from the left and right side-body paths.
...
| --- | --- | --- |
| `0..34` | arm | wrist to shoulder |
| `35..74` | leg | ankle to hip |
| `75..94` | side body | hip to underarm |
| `95..124` | chest half-spiral | outer ribs to solar plexus |
...
```text
WearablePoint
  output_index: int       # actual Twinkly frame index, 0..249
  path: "left" | "right"
  path_index: int         # 0..124 within the physical path
...
No animation should encode the 250 physical indices itself. Animations consume
...
1. Identify the factory output-index order of the two 125-dot paths with a
...
3. Add a wearable layout data file containing the 250 fitted points and a
...
- Exactly 250 points map one-to-one to the Twinkly output frame.
- Each physical path has exactly 125 recorded points.
